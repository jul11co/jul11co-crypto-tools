...
var _encryptFile = function(input_file, output_file, key, options, callback) {
  ...
  input_stream.on('error', function(err) {
    console.log('Read file failed: ' + input_file);
    console.log(err);
    return callback(err);
  });
  ...
  input_stream.on('end', function() {
    try {
      ...
      output_stream.on('error', function(err) {
        console.log('Write file failed: ' + output_file);
        console.log(err);
        return callback(err);
      });
...
var _decryptFilePart = function(input_file, offset, size, key, options, callback) {
  ...
  if (options.debug) console.log('_decryptFilePart:', 'offset:'+offset, 'size:'+size);
  ...
  if (options.debug) console.log('_decryptFilePart:', 'aligned_offset:'+aligned_offset, 'odd_offset:'+odd_offset);

  // first 16 bytes will be use to generate IV (if not start with offset 0)
  var data_offset = (aligned_offset >= 16) ? (aligned_offset - 16) : 0; 
  var data_size = (size + (aligned_offset - data_offset));
  var orig_size = data_size;
  data_size = (Math.ceil(data_size/16)*16); // make sure data size is a multiple of 16 bytes
  data_size = (data_size < 16) ? 16 : data_size;

  if (options.debug) console.log('_decryptFilePart:', 'data_offset:'+data_offset, 'data_size:'+data_size);
  ...
  if (options.debug) console.log('_decryptFilePart:', 'dec_buf.length:'+dec_buf.length);
  if (odd_offset > 0) {
    dec_buf = dec_buf.slice(odd_offset);
    if (options.debug) console.log('_decryptFilePart:', 'dec_buf.length:'+dec_buf.length, 'odd_offset:', odd_offset);
  }
  if (orig_size < data_size) {
    dec_buf = dec_buf.slice(0, orig_size);
    if (options.debug) console.log('_decryptFilePart:', 'dec_buf.length:'+dec_buf.length, 'data_size:', data_size);
  }
  dec_chunks.push(dec_buf);

  var final_buf = new Buffer(decipher.final('binary'), 'binary');
  if (options.debug) console.log('_decryptFilePart:', 'final_buf.length:'+final_buf.length);
...
function DecryptFileStream(input_file, key, opts) {
...
util.inherits(DecryptFileStream, Readable);

DecryptFileStream.prototype._read = function(size) {
  if (this._fd == -1) {
    this._fd = fs.openSync(this._input_file, 'r');
  }
  // TODO
}

var _createDecryptFileStream = function(input_file, key, options) {
  // TODO
}

exports.createDecryptFileStream = _createDecryptFileStream;
  ...
  scanDir(input_dir, options, function(err, files, dirs) {
    ...
    async.eachSeries(files, function(file, cb) {
      ...
      if (utils.fileExists(encrypted_file_path)) {
        ...
        var entry_hash = utils.md5Hash(orig_file_rel_path);
        var entry = crypto_index.get(entry_hash);
        if (!entry) {
          // 1. add missing entry
          crypto_index.put(entry_hash, {
          ...
          return cb();
        } else if (entry.s == file.size && entry.mt == file.mtime) {
  ...
  async.eachSeries(files, function(file, cb) {
    ...
    if (!utils.fileExists(encrypted_file_path)) {
      ...
      if (options.remove_encrypted_files) {
        crypto_index.remove(utils.md5Hash(orig_file.path));
      }
      return cb();
    }
...
var removeFiles = function(input_dir, file_list, encryption_key, crypto_index, options, done) {
  ...
  if (crypto_index.version() !== '0.0.1') {
    for (var key in crypto_index.map()) {
      ...
      var will_remove = file_list.some(function(entry) {
        return (file_info.p.indexOf(entry) == 0);
      });
  ...
  var onFileRemove = function(decrypted_file, encrypted_file) {
    if (typeof options.onFileRemove == 'function') {
      options.onFileRemove(decrypted_file, encrypted_file, {current: count, total: total});
    }
  }
  ...
  var onFileRemoveFailed = function(err, decrypted_file, encrypted_file) {
    if (typeof options.onFileRemoveFailed == 'function') {
      options.onFileRemoveFailed(err, decrypted_file, encrypted_file, {current: count, total: total});
    }
  }
