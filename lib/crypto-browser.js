...
module.exports = function(crypto_source, tmp_dir, options) {
  options = options || {};
  ...
  crypto_source.listEntry(list_opts, function(err, result) {
    if (err) {
      console.log('List crypto error!', crypto_source.path);
      console.log(err);
      process.exit();
    }
    ...
    dirs.forEach(function(dir) {
      ...
      var dir_relpath = dir.path;
        // (dir_path == data_dir) ? '.' : path.relative(data_dir, dir_path);

      // console.log('Directory:', dir_relpath);
      ...
      if (dir_relpath != '.' && dir_relpath != './') {
        ...
        if (directory_trailing_slash) parent_dir_relpath += '/';
          // (parent_dir_path == data_dir) ? '.' : path.relative(data_dir, parent_dir_path);

        // console.log('Parent:', parent_dir_relpath);
